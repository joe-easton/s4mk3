...
Item {
  ...
  AppProperty { id: sampleRate;      path: "app.traktor.decks." + deckInfo.deckId + ".track.content.sample_rate" }
  AppProperty { id: loopEnabled;     path: "app.traktor.decks." + deckInfo.deckId + ".loop.active"          }
  AppProperty { id: activeCueType;   path: "app.traktor.decks." + deckInfo.deckId + ".track.cue.active.type"        }
  AppProperty { id: activeCueStart;  path: "app.traktor.decks." + deckInfo.deckId + ".track.cue.active.start_pos"   }
  AppProperty { id: activeCueLength; path: "app.traktor.decks." + deckInfo.deckId + ".track.cue.active.length"      }
  AppProperty { id: activePos;       path: "app.traktor.decks." + deckInfo.deckId + ".track.cue.active.start_pos"   }
  AppProperty { id: loopSizePos;     path: "app.traktor.decks." + deckInfo.deckId + ".loop.size"                    }
  ...
  Repeater {
    ...
    Traktor.WaveformTranslator
    {
      ...
      AppProperty { id: pos;    path: "app.traktor.decks." + view.deckId + ".track.cue.hotcues." + (index + 1) + ".start_pos" }

      followTarget: waveformPosition
      pos:          pos.value * sampleRate.value // pos in samples

      Widgets.Hotcue {
        AppProperty { id: length; path: "app.traktor.decks." + view.deckId + ".track.cue.hotcues." + (index + 1) + ".length"    }
        anchors.topMargin: 3
        height:            view.height
